control "SV-216143" do
  title "The system must disable network routing unless required."
  desc "The network routing daemon, in.routed, manages network routing tables. If enabled, it 
...
Routing 
Internet Protocol (RIP) is a legacy protocol with a number of security weaknesses, including 
a lack of authentication, zoning, pruning, etc."
  desc "check", "Determine if routing is disabled. 

# routeadm -p | egrep \"routing |forwarding\" | grep 
enabled

If the command output includes \"persistent=enabled\" or \"current=enabled\", this 
is a finding."
...
# pfexec routeadm -d ipv6-forwarding -d 
ipv6-routing
...
routeadm -u"
  ...
  tag gid: "V-216143"
  tag rid: "SV-216143r603268_rule"
  tag stig_id: "SOL-11.1-050130"
  tag fix_id: "F-17379r372812_fix"
