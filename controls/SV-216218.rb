control "SV-216218" do
  title "The system must require authentication before allowing modification of the boot devices or 
menus. Secure the GRUB Menu (Intel)."
  desc "The flexibility that GRUB provides creates a security risk if its configuration is modified 
by an unauthorized user. The failsafe menu entry needs to be secured in the same environments 
that require securing the systems firmware to avoid unauthorized removable media boots."
...
If the output does not contain \"source $prefix/custom.cfg\" on a line 
of its own, this is a finding.

# grep superusers /rpool/boot/grub/custom.cfg.
# grep 
password_pbkdf2 /rpool/boot/grub/custom.cfg

If no superuser name and password are 
