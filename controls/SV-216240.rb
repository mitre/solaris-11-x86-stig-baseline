control "SV-216240" do
  title "The systems physical devices must not be assigned to non-global zones."
  desc "Solaris non-global zones can be assigned physical hardware devices. This increases the risk 
of such a non-global zone having the capability to compromise the global zone."
  desc "check", "This check applies to the global zone only. Determine the zone that you are currently 
securing.

# zonename

If the command output is \"global\", this check applies.

List the 
non-global zones on the system.

# zoneadm list -vi | grep -v global

List the configuration 
for each zone.

# zonecfg -z [zonename] info | grep dev

Check for device lines. If such a line 
exists and is not approved by security, this is a finding."
  desc "fix", "This check applies to the global zone only. Determine the zone that you are currently 
securing.

# zonename

If the command output is \"global\", this check applies.

The Zone 
Security profile is required:

Remove all device assignments from the non-global zone. 

# 
pfexec zonecfg -z [zone] delete device [device]"
  impact 0.5
  tag severity: "medium"
  tag gtitle: "SRG-OS-000480"
  tag gid: "V-216240"
  tag rid: "SV-216240r603268_rule"
  tag stig_id: "SOL-11.1-100030"
  tag fix_id: "F-17476r373097_fix"
  tag legacy: ["SV-60715","V-47841"]
  tag cci: ["CCI-000366"]
  tag nist: ["CM-6 b"]
end