control "SV-216191" do
  ...
  desc "User groups must be assigned unique GIDs for accountability and to ensure appropriate access 
protections."
  desc "check", "The root role is required.

Check that group IDs are unique.

# getent group | cut -f3 -d\":\" | 
sort -n | uniq -c |\\
while read x ; do
[ -z \"${x}\" ] &amp;&amp; break
set - $x
...
Work with each respective group owner to remediate this issue and 
ensure that the group ownership of their files are set to an appropriate value."
  impact 0.5
  tag severity: "medium"
