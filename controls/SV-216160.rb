control "SV-216160" do
  title "The GNOME service must display the DoD approved system use notification message or banner 
...
Log in to the Gnome Graphical 
interface. If the approved banner message does not appear, this is a finding.
...
If /etc/issue does not contain that 
DoD-approved banner message or /etc/gdm/Init/Default does not contain the 
line:
...
Insert the proper DoD banner message text. The DoD required text 
is:
...
Add the following content before the \"exit 0\" line of the 
file.
