control "SV-216158" do
  title "The operating system must display the DoD approved system use notification message or banner 
before granting access to the system for general system logons."
  desc "Warning messages inform users who are attempting to log in to the system of their legal status 
regarding the system and must include the name of the organization that owns the system and any 
monitoring policies that are in place. As implementing a logon banner to deter inappropriate 
use can provide a foundation for legal action against abuse, this warning content should be 
set as appropriate."
  desc "check", "Review the contents of these two files and check that the proper DoD banner message is 
configured.

# cat /etc/motd
# cat /etc/issue

If the DoD-approved banner text is not in 
the files, this is a finding."
  desc "fix", "The root role is required.

Edit the contents of these two files and ensure that the proper DoD 
banner message is viewable.
...
work product are private and confidential. See User Agreement for details.\""
  ...
  tag gid: "V-216158"
  tag rid: "SV-216158r603268_rule"
  tag stig_id: "SOL-11.1-050380"
  tag fix_id: "F-17394r372857_fix"
